#!/usr/bin/env python3
"""
Script directo para probar el sistema de menús del restaurante
"""
import requests
import json
from datetime import date

def test_restaurant_menu_system():
    """Probar el sistema completo de menús del restaurante"""
    ...
    try:
        ...
        # 3. Probar platos
        print("\n🍽️ Probando platos del restaurante...")
        try:
        ...
        # 7. Probar crear un plato de ejemplo
        print("\n🍽️ Probando creación de plato...")
        try:
            plato_data = {
                "nombre": "Plato de Prueba",
                "descripcion": "Plato creado para probar el sistema",
                "precio": 15000.0,
                "tipo": "Menu_Dia"
            }
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"✅ Plato creado: {data.get('nombre', 'N/A')} (ID: {data.get('id', 'N/A')})")
            else:
                print(f"❌ Error: {response.text}")
        except Exception as e:
            print(f"❌ Error creando plato: {e}")
        
        print("\n" + "=" * 60)
        print("🎉 Pruebas del sistema de menús del restaurante completadas!")
        ...
        print("  - ✅ POST /api/v1/restaurant-menu/platos/")
